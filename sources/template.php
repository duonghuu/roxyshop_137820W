...
if($source=="index"){
    ...
    $firstarticle=get_fetch("select noidung$lang as noidung from #_about 
        where type='first-article' ");
    $thongtinchinhsach=get_fetch("select noidung$lang as noidung from #_about 
        where type='thong-tin-chinh-sach' ");
    $product_danhmuc=get_result("select ten$lang as ten,tenkhongdau,id,type from
     #_product_danhmuc where type='san-pham' and hienthi>0 order by stt asc");
    $thuvien=get_result("select id,thumb,photo from #_hinhanh 
    ...
    $spnoibat=get_result("select ten$lang as ten,tenkhongdau,id,type,gia,giakm,thumb,photo from
     #_product where type='san-pham' and noibat>0 and hienthi>0 order by stt asc");
